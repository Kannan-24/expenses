...
class TransactionReportService
{
    ...
    private function validateReportRequest(Request $request): array
    {
        $rules = [
            ...
            'category_id' => 'nullable|in:all,' . Category::where('user_id', Auth::id())->pluck('id')->implode(','),
            'wallet_id' => 'nullable|in:all,' . Wallet::where('user_id', Auth::id())->pluck('id')->implode(','),
            'person_id' => 'nullable|in:all,' . ExpensePerson::where('user_id', Auth::id())->pluck('id')->implode(','),
