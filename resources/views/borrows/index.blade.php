<x-app-layout>
    <x-slot name="title">
        {{ __('Borrows & Lends Management') }} - {{ config('app.name', 'Cazhoo') }}
    </x-slot>
    ...
    <div class="min-h-screen">
        <div class="max-w-7xl mx-auto">
            ...
            <div
                ...
                <div
                    class="bg-gradient-to-br from-indigo-600 via-blue-700 to-blue-900 border-b border-blue-500 dark:border-blue-600 p-4 sm:p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2">Borrow & Lend
                                Management</h1>
                ...
                <form method="GET" class="space-y-4" id="form-filter">
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-3">
                        <div class="flex flex-wrap gap-2">
                            <button type="button"
                                @click="document.querySelector('select[name=type]').value='lent'; document.getElementById('form-filter').submit();"
                            ...
                            <button type="button"
                                @click="document.querySelector('select[name=type]').value='borrowed'; document.getElementById('form-filter').submit();"
                    ...
                    <div x-show="showAdvanced" x-transition
                        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                        <select name="type" class="hidden">
                            <option value="">All</option>
                            <option value="lent" {{ request('type') == 'lent' ? 'selected' : '' }}>Lent</option>
                            <option value="borrowed" {{ request('type') == 'borrowed' ? 'selected' : '' }}>Borrowed
                            </option>
                        </select>
                        <div>
                            <label
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Wallet</label>
                            <select name="wallet"
                                class="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                <option value="">All Wallets</option>
                                @foreach ($wallets as $wallet)
                                    <option value="{{ $wallet->id }}"
                                        {{ request('wallet') == $wallet->id ? 'selected' : '' }}>
                                        {{ $wallet->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Person</label>
                            <select name="person"
                                class="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            @empty
                                <tr>
                                    <td colspan="9"
                                        ...
                                        <a href="{{ route('borrows.create') }}"
                                            class="text-blue-600 dark:text-blue-400">Add one now</a>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                        {{ $borrow->person->name ?? '-' }}</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">#{{ $borrow->id }} •
                                        {{ ucfirst($borrow->borrow_type) }}</p>
                                </div>
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $borrow->borrow_type === 'lent' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                    {{ ucfirst($borrow->borrow_type) }}
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium text-gray-900 dark:text-white">
                                        {{ \Carbon\Carbon::parse($borrow->date)->format('M d, Y') }}</p>
                                </div>
                            </div>
                            <div
                                ...
                                <div class="flex items-center space-x-3">
                                    <a href="{{ route('borrows.show', $borrow->id) }}"
                                        class="inline-flex items-center text-yellow-600 dark:text-yellow-400 text-sm font-medium">View</a>
                                    <a href="{{ route('borrows.edit', $borrow->id) }}"
                                        class="inline-flex items-center text-blue-600 dark:text-blue-400 text-sm font-medium">Edit</a>
                                    <form action="{{ route('borrows.destroy', $borrow->id) }}" method="POST"
                                        class="inline-block" onsubmit="return confirm('Delete this borrow?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center text-red-600 dark:text-red-400 text-sm font-medium">Delete</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="p-8 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No borrows/lends
                                    found</h3>
                                <p class="text-gray-500 dark:text-gray-400 mb-4">Get started by adding your first
                                    borrow/lend.</p>
                                <a href="{{ route('borrows.create') }}"
                ...
                <div
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                        <div class="text-sm text-gray-700 dark:text-gray-300">
                            Showing {{ $borrows->firstItem() }} to {{ $borrows->lastItem() }} of
                            {{ $borrows->total() }} records
                        </div>
                        <div>
