<x-app-layout>
    <x-slot name="title">
        {{ __('Borrow Details') }} - {{ config('app.name', 'Cazhoo') }}
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex items-center">
                            ...
                            @elseif($borrow->status === 'partial')
                                <div
                                    class="inline-flex items-center px-4 py-2 bg-yellow-100 dark:bg-yellow-900 rounded-full">
                                    ...
                                    <span class="text-yellow-800 dark:text-yellow-200 font-semibold">Partially
                                        Returned</span>
                                </div>
                            @else
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div
                    ...
                    <div class="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-600 p-4 sm:p-6">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Complete details of this
                                    borrow/lend entry</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 sm:p-6">
                        <div class="space-y-6">
                            <div
                                class="text-center p-6 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-600 mb-6">
                                <p class="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Borrowed/Lent
                                    Amount</p>
                                <p
                                    class="text-4xl font-bold {{ $borrow->borrow_type === 'lent' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                ...
                                <span
                                    class="text-sm text-gray-600 dark:text-gray-400 font-mono">#{{ $borrow->id }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Return
                                    Records</span>
                                <span
                                    class="text-sm text-gray-600 dark:text-gray-400">{{ $borrow->histories->count() }}
                                    total</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <div class="mt-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden"
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Track all repayments/returns
                                    for this borrow</p>
                            </div>
                        </div>
                        @if ($borrow->status !== 'returned')
                ...
                @if ($borrow->status !== 'returned')
                    <div x-show="openReturnModal"
                        ...
                        <div @click.away="openReturnModal = false"
                            class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
                            <div class="flex justify-between items-center mb-4">
                                <h3 class="text-xl font-bold text-gray-900 dark:text-white">Record Repayment / Return
                                </h3>
                            ...
                            <form action="{{ route('borrows.repay', $borrow->id) }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 gap-6">
                                    <div>
                                        <label class="font-bold text-sm text-gray-900 dark:text-white">Amount
                                            Returned</label>
                                        <input type="number" name="repay_amount" min="0.01"
                                            max="{{ $borrow->amount - $borrow->returned_amount }}" step="0.01"
                                            required
                                            class="w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-medium shadow-sm"
                                            placeholder="Amount">
                                    </div>
                                    <div>
                                ...
                                <div class="mt-6 flex justify-end">
                                    <button type="button" @click="openReturnModal = false"
                                        class="px-4 py-2 text-gray-900 dark:text-white bg-gray-200 dark:bg-gray-700 rounded-xl mr-2">Cancel</button>
                                    <button type="submit"
                                        class="px-8 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl">Record
                                        Return</button>
                                </div>
                            </form>
                        </div>
                ...
                @foreach ($borrow->histories as $history)
                    <div x-show="openEditModal === {{ $history->id }}"
                        class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
                        style="display: none;">
                        <div @click.away="openEditModal = null"
                            class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
                            <div class="flex justify-between items-center mb-4">
                            ...
                            <form action="{{ route('borrows.return.update', [$borrow->id, $history->id]) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                <div class="grid grid-cols-1 gap-6">
                                    <div>
                                        ...
                                        <input type="number" name="amount" min="0.01"
                                            max="{{ $borrow->amount - $borrow->histories->where('id', '!=', $history->id)->sum('amount') }}"
                                            step="0.01" required value="{{ $history->amount }}"
                                            class="w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-medium shadow-sm"
                                            placeholder="Amount">
                                    </div>
                                    <div>
                                        ...
                                        <select name="wallet_id"
                                            ...
                                            @foreach ($wallets as $wallet)
                                                <option value="{{ $wallet->id }}"
                                                    {{ $history->wallet_id == $wallet->id ? 'selected' : '' }}>
                                                    {{ $wallet->name }}
                                                    ({{ $wallet->currency->symbol }}{{ number_format($wallet->balance, 2) }})
                                                </option>
                                            @endforeach
                                    ...
                                    <div>
                                        <label class="font-bold text-sm text-gray-900 dark:text-white">Date</label>
                                        <input type="date" name="date" required value="{{ $history->date }}"
                                            class="w-full px-4 py-3 border-2 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-medium shadow-sm">
                                    </div>
                                </div>
                                <div class="mt-6 flex justify-end">
                                    <button type="button" @click="openEditModal = null"
                                        class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-xl mr-2">Cancel</button>
                                    <button type="submit"
                                        class="px-8 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl">Update
                                        Return</button>
                                </div>
                            </form>
                        </div>
                                    ...
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No
                                                return history found</h3>
                                            <p class="text-gray-500 dark:text-gray-400">No repayments/returns have been
                                                recorded for this borrow.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    @forelse ($borrow->histories as $history)
                        <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex flex-col gap-2">
                            <div class="flex items-center justify-between mb-1">
                                <div>
                                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">Return
                                        #{{ $loop->iteration }}</h4>
                            ...
                            <div>
                                ...
                                <span
                                    class="font-medium text-gray-900 dark:text-white">{{ $history->wallet->name ?? '-' }}</span>
                            </div>
                            <div class="mt-2">
                                <button @click="openEditModal = {{ $history->id }}"
                    ...
                    @empty
                        <div class="p-8 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No return history
                                    found</h3>
                                <p class="text-gray-500 dark:text-gray-400">No repayments/returns have been recorded
                                    for this borrow.</p>
                            </div>
                        </div>
                    @endforelse
