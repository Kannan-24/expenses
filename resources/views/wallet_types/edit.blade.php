<x-app-layout>
    <x-slot name="title">
        {{ __('Edit Wallet Type') }} - {{ config('app.name', 'expenses') }}
    </x-slot>
    ...
    <div class="sm:ml-64">
        <div class="w-full mx-auto max-w-7xl sm:px-6 lg:px-8 bg-white p-6 rounded-2xl shadow m-4 flex flex-col" style="height: 88vh;">
                ...
                <nav class="flex text-sm text-gray-500" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li class="flex items-center">
                            <a href="{{ route('wallet-types.index') }}" class="inline-flex items-center hover:text-blue-600">
                        ...
                        <li class="flex items-center">
                            ...
                            <span class="text-gray-700">Edit Wallet Type</span>
                        </li>
                    </ol>
                </nav>
            </div>
            ...
            <!-- Form -->
            <form action="{{ route('wallet-types.update', $walletType->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-5 mt-3">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $walletType->name) }}"
                ...
                <div class="mb-5">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full p-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $walletType->description) }}</textarea>
                    @error('description')
                        <span class="text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-5 flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                        class="mr-2" {{ old('is_active', $walletType->is_active) ? 'checked' : '' }}>
