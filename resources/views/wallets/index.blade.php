<x-app-layout>
    <x-slot name="title">
        {{ __('Wallet Management') }} - {{ config('app.name', 'Cazhoo') }}
    </x-slot>
    ...
    <div class="min-h-screen">
        <div class="max-w-7xl mx-auto">
                            ...
                            <a href="{{ route('wallets.transfer.form') }}"
                                ...
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" stroke-width="2"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M17 9V7a5 5 0 00-10 0v2a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2zm-7 4h6">
                                    </path>
                                </svg>
                                Transfer Funds
                            </a>
            ...
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 mb-6 p-4 sm:p-6">
                <form method="GET" class="space-y-4" id="filter-form" x-data="{ showAdvanced: false }">
                    ...
                    <div class="relative max-w-2xl mx-auto">
                        ...
                        @php
                            $hasFilters =
                                request('search') || request('filter') || request('wallet_type') || request('currency');
                        @endphp
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button type="button"
                                @click="document.querySelector('select[name=filter]').value='active'; document.querySelector('filter-form').submit();"
                            ...
                            <button type="button"
                                @click="document.querySelector('select[name=filter]').value='inactive'; document.querySelector('filter-form').submit();"
                        ...
                        <button type="button" @click="showAdvanced = !showAdvanced"
                            ...
                            Advanced Filters
                            <svg x-show="!showAdvanced" class="w-4 h-4 ml-2" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div x-show="showAdvanced" x-transition
                        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                        <!-- Quick Filter Hidden Select -->
                        <select name="filter" class="hidden">
                            <option value="">All</option>
                            <option value="active" {{ request('filter') == 'active' ? 'selected' : '' }}>Active
                            </option>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                @if (request('search'))
                                    Showing results for "<span
                                        class="font-semibold text-blue-600 dark:text-blue-400">{{ request('search') }}</span>"
                            ...
                            @forelse ($wallets as $wallet)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center">
                                            <span
                                                class="font-medium text-gray-900 dark:text-white">{{ $wallet->currency->code }}</span>
                                            <span
                                                class="ml-2 text-gray-500 dark:text-gray-400">{{ $wallet->currency->symbol }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $wallet->balance >= 0 ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                            {{ $wallet->currency->symbol }}{{ number_format($wallet->balance, 2) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $wallet->is_active ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                            @if ($wallet->is_active)
                                        ...
                                        <div class="flex flex-col items-center">
                                            ...
                                            @if (request('search') || $hasFilters)
                                                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
                                                    No wallets found</h3>
                                                <p class="text-gray-500 dark:text-gray-400 mb-4">No wallets match your
                                                    search criteria.</p>
                                                <a href="{{ route('wallets.index') }}"
                        ...
                        <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <p
                                        class="font-bold text-lg {{ $wallet->balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                        {{ $wallet->currency->symbol }}{{ number_format($wallet->balance, 2) }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Currency</p>
                                    <p class="font-medium text-gray-900 dark:text-white">{{ $wallet->currency->code }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <form action="{{ route('wallets.destroy', $wallet->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center text-red-600 dark:text-red-400 text-sm font-medium">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                    ...
                    @empty
                        <div class="p-8 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                @if (request('search') || $hasFilters)
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No wallets
                                        found</h3>
                                    <p class="text-gray-500 dark:text-gray-400 mb-4">No wallets match your search
                                        criteria.</p>
                                    <a href="{{ route('wallets.index') }}"
                                        class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">
                                        Clear Search
            ...
            @if ($wallets->hasPages())
                <div
                    class="mt-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
