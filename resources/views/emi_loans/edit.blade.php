<x-app-layout>
    <x-slot name="title">
        {{ __('Edit EMI Loan') }} - {{ config('app.name', 'Expense Tracker') }}
    </x-slot>
    ...
    <div class="min-h-screen">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 mb-6 overflow-hidden">
                <div class="bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 dark:from-purple-800 dark:via-purple-900 dark:to-indigo-900 border-b border-purple-500 dark:border-purple-600 p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2">Edit EMI Loan</h1>
                            <nav class="flex text-sm" aria-label="Breadcrumb">
                                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                                    ...
                                    <li class="flex items-center">
                                        ...
                                        <span class="text-purple-100 font-medium">Edit</span>
                                    </li>
                                </ol>
                            </nav>
                        </div>
                        ...
                        <div class="flex items-center space-x-3">
                            <a href="{{ route('emi-loans.show', $emiLoan) }}"
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
                <form method="POST" action="{{ route('emi-loans.update', $emiLoan) }}" class="space-y-6" x-data="emiCalculator()">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $emiLoan->name) }}" required
                            ...
                            <div>
                                ...
                                <select name="category_id" id="category_id"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_id', $emiLoan->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="default_wallet_id" id="default_wallet_id"
                                    ...
                                    @foreach($wallets as $wallet)
                                        <option value="{{ $wallet->id }}" {{ old('default_wallet_id', $emiLoan->default_wallet_id) == $wallet->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="total_amount" id="total_amount" value="{{ old('total_amount', $emiLoan->total_amount) }}" 
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="interest_rate" id="interest_rate" value="{{ old('interest_rate', $emiLoan->interest_rate) }}" 
                            ...
                            <div>
                                ...
                                <input type="number" name="tenure_months" id="tenure_months" value="{{ old('tenure_months', $emiLoan->tenure_months) }}" 
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $emiLoan->start_date->format('Y-m-d')) }}" required
                                       class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200">
                                @error('start_date')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="loan_type" id="loan_type" x-model="loanType" @change="calculateEMI()" required
                                    ...
                                    <option value="fixed" {{ old('loan_type', $emiLoan->loan_type) == 'fixed' ? 'selected' : '' }}>Fixed Rate</option>
                                    <option value="reducing_balance" {{ old('loan_type', $emiLoan->loan_type) == 'reducing_balance' ? 'selected' : '' }}>Reducing Balance</option>
                                </select>
                                @error('loan_type')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                        class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                    <option value="active" {{ old('status', $emiLoan->status) == 'active' ? 'selected' : '' }}>Active</option>
                                    <option value="closed" {{ old('status', $emiLoan->status) == 'closed' ? 'selected' : '' }}>Closed</option>
                                    <option value="cancelled" {{ old('status', $emiLoan->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="monthly_emi" id="monthly_emi" value="{{ old('monthly_emi', $emiLoan->monthly_emi) }}" 
                                           step="0.01" min="0"
                                           class="w-full pl-8 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" name="is_auto_deduct" id="is_auto_deduct" value="1" 
                                       {{ old('is_auto_deduct', $emiLoan->is_auto_deduct) ? 'checked' : '' }}
    ...
    <script>
        function emiCalculator() {
            return {
                totalAmount: {{ old('total_amount', $emiLoan->total_amount) }},
                interestRate: {{ old('interest_rate', $emiLoan->interest_rate) }},
                tenureMonths: {{ old('tenure_months', $emiLoan->tenure_months) }},
                loanType: '{{ old('loan_type', $emiLoan->loan_type) }}',
                calculatedEMI: 0,
