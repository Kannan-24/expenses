<!-- Add x-data to the parent for Alpine.js state -->
<div x-data="{ sidebarOpen: false }">
    <!-- Top Navigation Bar -->
    <nav class="fixed top-0 z-50 w-full p-1 bg-white border-b border-gray-200 shadow">
        <div class="px-4 py-2 flex items-center justify-between">
            ...
            <!-- Right: Welcome, Notification, Settings, Avatar -->
            <div class="flex items-center space-x-4">
                <!-- Profile Dropdown (Avatar only, optional dropdown like before) -->
                <div class="relative" x-data="{ dropdownOpen: false }">
                    <a href="{{ route('profile.show') }}" class="focus:outline-none flex items-center space-x-2">
                        @if (Auth::user()->profile_photo)
                            <img class="w-10 h-10 rounded-full" src="{{ asset('storage/' . Auth::user()->profile_photo) }}"
                                alt="User photo" />
                        @else
                            <div
    ...
    <div x-show="sidebarOpen" x-transition.opacity
        class="fixed inset-0 z-30 bg-black bg-opacity-40 sm:hidden"
    ...
    <aside id="logo-sidebar"
        ...
        class="fixed top-0 left-0 z-40 w-64 h-screen pt-16 transition-transform bg-white border-r border-gray-200
